...
export default class Nav extends Component {
  ...
  handleLogin(e) {
    ...
    var this1 = this
    if (
      document.getElementsByClassName('input-param')[0].value === '' ||
      document.getElementsByClassName('input-param')[1].value === ''
    ) {
        ...
        <header className={style.header}>
          ...
          {this.state.isLogin ? (
            <span className={style.text}>
              <Tag>Hello,{window.localStorage.getItem('accountName')}</Tag>
          ...
          <Modal
            ...
            onCancel={this.handleLoginCan}
            okText="登录"
            cancelText="取消"
          >
            <div className="userInfo">
              <label> 账户名:</label> <Input className="input-param" />
            </div>
            <div className="userInfo">
              <label> 密码：</label>{' '}
              <Input className="input-param" type="password" />
            </div>
          </Modal>
        </header>
